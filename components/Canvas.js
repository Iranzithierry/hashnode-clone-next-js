...
export default function Canvas({ onClose, showSidebar }) {
    ...
    return (
        <div>
            ...
            <div className={`z-40 w-[75%] flex sm:hidden dark:bg-dark-1 bg-gray-0 fixed left-0 top-0 bottom-0 transition-all duration-600 ${width}`}>
                <div className='h-full  w-full flex flex-col justify-items-center space-x-2 py-6 px-4'>
                    ...
                    <div className='flex flex-1 flex-col justify-center'>
                        ...
                        <ul className='flex flex-col  justify-center items-start pt-2 space-y-8'>
                            {items.map((item, index) => {
                                ...
                                return (
                                    <a href='#' key={index} className='w-full text-start focus:ring-2 ring-primary-0 ring-offset-2 dark:ring-offset-transparent rounded-lg p-1.5'>
                                        <li className={`text-slate-700 dark:text-neutral-200 text-lg font-inter ${isSelected ? 'font-bold !text-primary-0' : 'font-semibold'}`} key={index}>{item}</li>
                                    </a>
                                )
                            })}
                        </ul>
